Public Class frmRFIDUpdate
    ...
    Protected Friend Sub loadClientInfo(ByVal playerID As Integer)
        SQLstr = String.Format("SELECT PlayerID, LName, FName, MI, PlayerPic, RFIDNum FROM Player WHERE PlayerID = {0}", playerID)
        sqlQueryCMD(SQLstr)
        ...
        If rdDB.HasRows Then
            ...
            If Len(rdDB!RFIDNum.ToString) > 0 Then
                txtRFIDnum.Text = rdDB!RFIDNum
    ...
    Private Function isRFIDAvail(ByVal rfid As String) As Boolean
        ...
        If Not rdDB.HasRows Then
            lblIDstatus.Text = "RFID can now be assigned to the client"
            picStatIndicator.Image = My.Resources.icons8_green_check_mark_2
            isRFIDAvail = True
        Else
            rdDB.Read()
            lblIDstatus.Text = String.Format("RFID is already assigned to {0} {1}", rdDB!Fname, rdDB!Lname)
    ...
    Private Sub frmRFIDUpdate_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        e.Handled = False
    End Sub
    ...
    Private Sub btnConfirm_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnConfirm.Click
        SQLstr = String.Format("UPDATE Player SET RFIDnum = '{0}' WHERE PlayerID = {1}", txtRFIDnum.Text, lblPlayerID.Text)
        Try
            sqlNonQueryCMD(SQLstr)
            MessageBox.Show("The RFID is now assigned!")
            clearFields()
            Me.Close()
